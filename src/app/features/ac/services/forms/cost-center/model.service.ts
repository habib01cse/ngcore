...
function getMenu(parentID, data) {
  ...
  return fnData.filter(function (node) {
      ...
      if (node.CHK_LEAF === 0) {
          if (node.LEVEL === 1) {
              return '<li><a href="#" class="tree-menu-text has-submenu leaf"><i class="fa fa-plus-square-o collapse-icon"></i><span>' +
               node.COST_NAME + '</span></a>' + 
               subMenu + '<div class="action-buttons"><a class="add-button" rel="' +
               jsonData + '"><i class="ion-plus"></i></a></div></li>';
          }
          else {
              return '<li><a href="#" class="tree-menu-text has-submenu leaf"><i class="fa fa-plus-square-o collapse-icon"></i><span>' +
               node.COST_NAME + '</span></a>' + 
               subMenu + '<div class="action-buttons"><a class="edit-button" rel="' + 
               jsonData + '" ><i class="ion-edit"></i></a><a class="add-button" rel="' + 
               jsonData + '" ><i class="ion-plus"></i></a><a class="delete-button" rel="' + 
               jsonData + '" ><i class="ion-trash-a"></i></a></div></li>';
          }
      }
      else {
          return '<li><a href="#" class="tree-menu-text last-leaf leaf" ><span>' + 
          node.COST_NAME + '</span></a><div class="action-buttons"><a class="edit-button" rel="' + 
          jsonData + '" ><i class="ion-edit"></i></a><a class="add-button" rel="' + 
          jsonData + '" ><i class="ion-plus"></i></a><a class="delete-button" rel="' + 
          jsonData + '" ><i class="ion-trash-a"></i></a></div></li>'
      }
...
export class ModelService{
  ...
  init(that) {
    ...
    // Without api  
	let data = JSON.parse( this.storageService.getData( globalVariables.DAMI_COST_CNETER_TREE)); 
    console.log('data', data); 
    
    data.map(item => {
        item['SQL_STATE'] = fixedValues.sqlState.sqlUpdate;
    })
    this.htmlStr = getMenu(null, data).join('');
    let _this = this;
    setTimeout(function () {
        that.loadScript();
    }, 500);
