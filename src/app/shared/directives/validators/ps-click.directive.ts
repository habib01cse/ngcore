...
import { PsValidateDirective } from './ps-validate.directive';
...
export class PsClickDirective {

  @Input('not') excludeText: string;
  @Input('group') groupText: string;

  @Output('ps-click') _validateEvent: EventEmitter<any> = new EventEmitter();
  ...
  public validate() {
    ...
    for (let index = 0; index < maps.length; index++) {
      if(maps[index].hasOwnProperty("psValidator")) {
        const _result_ = maps[index]['psValidator'].callValidation();
        if (!_result_.isValid) {
  ...
  public reset() {
    ...
    for (let index = 0; index < maps.length; index++) {
      if(maps[index].hasOwnProperty("psValidator")) {
        maps[index]['psValidator'].psResetValidation();
      }
    }
    return this.validationResult;
  }
