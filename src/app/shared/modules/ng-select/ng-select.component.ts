...
export class NgSelectComponent implements OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor  {
	...
	ngAfterViewInit() {
		...
		if (isDefined(this.autoFocus) && !this.autoFocusOff) {
			this.focus();
		}
	}
	...
	close() {
		...
		this._cd.markForCheck();
		this.element.querySelector('.ps-modal-open-wrapper').classList.remove('ng-select-remove');
	}
	...
	onClickClear(){
		...
		this.element.querySelector('.ps-modal-open-wrapper').classList.add('ng-select-remove');
		this.detectChanges();
	...
	private _setItemsFromNgOptions() {
		const handleNgOptions = (options: QueryList<NgOptionComponent>) => {
			this.items = options.map(option => ({
				...
				$ngOptionLabel: option.elementRef.nativeElement.innerText,
